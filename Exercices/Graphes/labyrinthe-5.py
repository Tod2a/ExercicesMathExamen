...
def peres_vers_graphe(peres):
    ...
    for x in peres:
        ...
        if x in G: 
            G[x].append(peres[x])
            print("\nx est déjà dans le graphe,on ajoute ses enfants", peres[x]," au graphe G")
            print("G=",G) 
        else: 
            G[x] = [peres[x]]
            print("\nx n'est pas encore dans le graphe,on ajoute x et ses enfants", peres[x]," au graphe G")
            print("G=",G)
        if peres[x] in G: 
            G[peres[x]].append(x)
            print("\nperes[x],",peres[x],"est déjà dans G, On rajoute x à la fin de peres[x],\nG=",G)
        else: 
            G[peres[x]] = [x]
            print("\nperes[x],",peres[x],"n'est pas déjà dans G, On le rajoute avec ses enfants,\nG=",G)
    return G
