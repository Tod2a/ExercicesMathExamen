...
def plot_labyrinthe(peres, M, N, lab=True):
    ...
    for (x, y) in peres:
        (a, b) = peres[(x, y)]
        if a == x + 1: plt.plot((x+d,x+d),(y-d,y+d),'w')				# effacement des côtés qui doivent laisser passer le visiteur en fonction des pères
        elif a == x - 1: plt.plot((x-d,x-d),(y-d,y+d),'w')				# c'est à dire s'il y a une arête d'un sommet vers un autre sommet (dictionnaire des pères !) 
